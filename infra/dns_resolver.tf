...
# clients via customDnsServers on the VPN gateway (vpn_gateway.tf).
# This is set at the gateway level (not the VNet level) because
# VNet-level custom DNS breaks the gateway's control plane.
# ---------------------------------------------------------
