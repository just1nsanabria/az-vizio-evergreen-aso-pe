...
# Hub management DNS zone for spoke-02 private endpoint (split-horizon DNS).
#
...
#   Hub mgmt zone (rg-eus2-hub-evergreen-01)  → linked to hub VNet (below)
...
resource "azurerm_private_dns_zone" "spoke2_aks_hub" {
  name                = "privatelink.eastus2.azmk8s.io"
  resource_group_name = azurerm_resource_group.hub.name
}

# Link the hub mgmt zone to the hub VNet so the DNS Private Resolver, hub pods,
# and VPN clients resolve aks-eus2-spoke-02.privatelink.eastus2.azmk8s.io to
# the PE NIC IP. The BYO zone is NOT linked here.
resource "azurerm_private_dns_zone_virtual_network_link" "spoke2_aks_hub" {
  name                  = "link-hub-vnet-spoke2"
  resource_group_name   = azurerm_resource_group.hub.name
  private_dns_zone_name = azurerm_private_dns_zone.spoke2_aks_hub.name
